...
namespace OtelProject.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        public async Task<ActionResult> Index(string searchString, int pricemin=1, int pricemax=0, byte countries = 0)
        {
            ...
            {
                otelCountry.Otels = await context.Otels.Where(a => a.OtelPrice >= pmin && a.OtelPrice <= pmax && a.OtelCountry == countries).ToListAsync();
            }

            //OtelName and Country
            else if (!(countries == 0) && !(string.IsNullOrEmpty(searchString)))
            {
                otelCountry.Otels = await context.Otels.Where(a => a.OtelName.ToLower().Contains(searchString.ToLower()) && a.OtelCountry == countries).ToListAsync();
            }
            //Onlys
            //Only price range
            else if ((pmax > pmin) && (countries==0) && (string.IsNullOrEmpty(searchString)))
            {
                otelCountry.Otels = await context.Otels.Where(a => a.OtelPrice >= pmin && a.OtelPrice <= pmax).ToListAsync();
            }

            //Only Country
            else if (!(countries == 0) && (string.IsNullOrEmpty(searchString)) && (pmax < pmin))
            {
                otelCountry.Otels = await context.Otels.Where(a => a.OtelCountry == countries).ToListAsync();
            }
            
            //Only OtelName
            else if(!(string.IsNullOrEmpty(searchString)) && (countries==0) && (pmax < pmin))
            {
                otelCountry.Otels = await context.Otels.Where(a => a.OtelName.ToLower().Contains(searchString.ToLower())).ToListAsync();
            }
            
            else
            {
                otelCountry.Otels = await context.Otels.ToListAsync();
            }
            ...
            otelCountry.Countries = await context.Countries.ToListAsync();
